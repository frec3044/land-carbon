...
- Each time a tree (individualID) is measured the date of measurement is recorded (Year), the status of the live (is it alive or dead), and the diameter (in centimeters) of the stem at breast height (stemDiameter). This is saved in the individual table.   
...
Add code to calculate the total tree carbon for each at each site
...
Generate a table of values for the site level carbon stocks averaged across years and plots (I recommend using the `kable()` function to generate a clean looking table).
...
Describe any issues that you had with your analysis, concerns you have with your analysis, and concerns you have with the data.

**Answer 5:**
  
**Question 6**

Describe whether your data analysis supported your hypothesis from Part #1 and, if not, possible reasons why it did not.
