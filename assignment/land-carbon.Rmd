...
There are many types of joins: left_join, right_join, inner_join full_join that differ in how they handle combining the tables. 
...
**Question 3:** Using the table from Question 2, calculate the carbon of each tree that is alive in each `plotID` in each `year` following the instructions provide at the top of Part 4.
...
**Question 9:** Generate a table of carbon stocks values for each site (averaged across years). I recommend using the `kable()` function to generate a clean looking table.
