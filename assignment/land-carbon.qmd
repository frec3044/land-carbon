...
Based on information about location and ecosystem type, rank the sites in order of 1 = most tree vegetation carbon per unit area, 4 = least tree vegetation carbon per unit area. Carbon per unit area is the average carbon for the same area at each sites (e.g., m2 of land).  *Describe the reasons why you choose the order.*
...
Typical RAM on a laptop is 8 - 16 GB and Hard Disk is 500 GB - 1 TB so you can store more on your hard disk than you can analyze in R.
...
The connection does not load any data into memory for you to analyze. The connections allows you to start looking at the database before loading data into memory.\
...
Fortunately, R uses the concept of "lazy loading". Look at the `individual` table to get a preview of the table that looks like the table.
...
Using the table from Question 2, create a new table that only has live trees and includes a column that is the carbon of each tree in each `plotID` in each `year` following the instructions provide at the top of Part 4. Use the function `colnames()` to show the column names of your table.
...
Generate a table of carbon stocks values for each site (averaged across years). Use the `kable()` function to generate a clean looking table.
