...
Provide a link to your assignment GitHub repository here:
...
The total biomass of a tree is the sum of aboveground and belowground carbon. Since belowground carbon is less commonly measured (it requires digging up tree roots), we can use ratios of aboveground to belowground carbon from a limited set of trees in which the entire tree was extracted.  The ratio is used to estimate belowground carbon in trees where we only have diameter measurements.
...
I am a client interested in carbon storage in vegetation across different ecosystems across the continental U.S. to inform my investment in the [California Carbon Market](https://ww2.arb.ca.gov/our-work/programs/compliance-offset-program).
...
Explore the descriptions of our four focal NEON sites at <https://www.neonscience.org/field-sites/field-sites-map/list>, how the sites look on the satellite image in the site description, and your general knowledge about ecosystem science to develop your hypotheses for the following question:

**Question 1:** 

Based on information about location and ecosystem type, rank the four focal sites in order of 1 = most tree vegetation carbon per unit area, 4 = least tree vegetation carbon per unit area. Carbon per unit area is the average carbon for the same area at each site (e.g., m2 of land).  *Describe the reasons why you chose the order.*

**Answer 1**

Provide your answer by the next Thursday class period on Canvas Assignment 6.1.
...
Working with data from NEON and many other sources requires handling data spread across multiple tables. For example, one table may include climate data for a set of focal locations, while another may include data collected during a site visit. Higher-level analyses that use the climate data to understand the field data require linking the two tables.

Furthermore, environmental data can be "big data." There are many definitions of big data, but I will use a simple one: the computer memory (RAM, not the hard disk) required to store your data is greater than your computer's capacity. When using this "big data," your computer will crash if you try to load or read all the data at once.
...
The database is a collection of data tables stored in a single file called `neon.duckdb` in the `assignment/data` subdirectory.
...
First, you need to [connect](%5Bhttps://r4ds.hadley.nz/databases.html#in-this-book%5D) to the database using the `dbConnect` function in the `DBI` package. You need to specify the database type (`duckdb()`) and the directory of the database file.
...
The connection does not load any data into memory for you to analyze. The connection lets you start inspecting the database before loading data into memory.\
...
Fortunately, R uses the concept of "lazy loading." Look at the `individual` table to get a preview of what the table looks like.

```{r}
individual
```

However, if you look more closely, it doesn't include all the rows. The preview only has 1000 rows. In fact, it does not have any rows

```{r}
nrow(individual)
```

This is because the data won't be loaded into memory until you explicitly tell it to (hence being "lazy"). The [`collect()` function](https://r4ds.hadley.nz/databases.html#dbplyr-basics) is what finally pulls the data into memory. In the example below, you can see that there are now many rows of data.
...
However, "collecting" the entire data table does not leverage the power of a database, as it pulls the entire table into memory. The advantage of using a database is that you can load only the data you actually need. In many cases, you don't need all rows or columns of a data table for your analysis.
...
You can `filter` only the rows you want before calling `collect`. Any filtering or selecting before a collect call will reduce the size of the data that you are actually moving from the hard disk to RAM. The following uses the `%in%` function to ask to keep data from any of the three `siteID` values in the vector.
...
Data in databases are spread across tables. This is done to help reduce the size of the data. For example, if every tree is in a site and each site is in a domain, do we need a `domainID` column in the same table as the individual trees? We could, but adding an extra column with a lot of repeated information (e.g., if there are 90 sites but three domains, the same three domains will be repeated over and over in the column) would unnecessarily use storage and memory. Alternatively, we have the `siteID` in the tree table, and another table with the `domainID` each site belongs to. This would reduce the database's storage requirement. Therefore, databases use multiple tables.

However, what if you want to filter the trees by which domain they are in? What if you want to average the trees at the domain scale? To do this, you actually need a domain column in the same table as your trees. "Joining" is how you combine two tables.

To explore joins, load another table (`domains`) and filter to the desired sites.
...
Following the left join, we can see a new column named `domainID`.
...
When you are finished pulling data from the database, it is good practice to close it using the following code:
...
You could filter on the presence of the word "Virginia" in the university variable. To do this, you can use a function from the [`stringr` package](https://r4ds.hadley.nz/strings.html) called `str_detect()`. The function returns TRUE or FALSE depending on whether the variable's value contains that string. You then filter using the function

```         
filter(str_detect(university, "Virginia"))
```

In the NEON data, each tree has an `individualID`. Imagine that we did not have our table with `domainID`, but you only wanted trees from domain 7 (`D07`). Notice that `individualID` is a string, and the string contains `domainID` (the example below is from the first row of the data).
...
The database you will be analyzing has five tables, but you will focus on four of them (the domain table was included only for the demonstration above). The structure of the data is described in the figure below. The arrows designate the different "keys" that link across the tables. The other variables are the data in the tables that you will use for the analysis.

![database structure](img/database_structure.png)

-   When an individual tree is first measured in a plot (`plotID`), it is given an `individualID` and a species identification (`taxonID`). This is saved in the mapping_tagging table.\
...
- Across the U.S., researchers have measured tree diameters and cut down the trees to weigh them. For each species (`taxonID`), they then develop equations that use diameter (in centimeters) to predict the weight (biomass). The equation has two parameters (`B0` and `B1`). This is saved in the allometrics table. The figure below illustrates the relationship between diameter and biomass. The equation for converting diameter to biomass is at the bottom of Table 4 in Jenkins et al. 2003.
...
-   The `plantStatus` column indicates whether the tree was alive at the time of measurement, but there are multiple types of live trees. Be sure your analysis includes all the types of live trees. You may need to use string manipulation and filter functions in the `stringr` package.\
-   The Climate Action Reserve project (an official carbon accounting organization for the California Carbon Exchange) provides allometric relationships to use to calculate **aboveground biomass**. The allometric equations for each species are listed in the "allometrics" table. You will need to **join** the parameters (B0 and B1) in "allometrics" to your table with the diameter measurements. The parameters we use come from Table 4 in Jenkins et al. 2003 in the assignments directory. The equation for converting diameter to biomass is at the bottom of Table 4 in Jenkins et al. 2003.
-   In R, the log(x) function is the natural log (base e) of x and the exp(x) function is e^x.
...
The **site level** value for each year is the mean of the plots at the site. Remember that we are only interested in the "Tower" plots.
...
Please remember to take a look at the description in the **Background on estimating carbon stocks** section on how to calculate carbon stocks.

Calculate the vegetation carbon stocks of each site for each year using the following steps: 

**Question 2:** 

Using join functions and different tables in the database to create a new table with the following columns: `siteID`, `plotID`, `individualID`, `taxonID`, `year`, `plantStatus`, `stemDiameter`, `B0`, `B1`.  Only include the four focal sites. Use the function `colnames()` to show the column names of your table.  
...
Using the table from Question 4, use a join to associate a plot area and plot type with each plot. Your columns should be: `siteID`, `plotID`, `carbon`, `plotType`, `totalSampledAreaTrees`, and `year`. Use the function `colnames()` to show the column names of your table.
...
Using the table for Question 6, average the plots to obtain a carbon per area value for each year at each site.
...
Generate a figure showing the mean live tree carbon stocks in the site and year. You will have years on the x-axis and carbon stocks on the y-axis, with different lines for each site. Be sure your figures are complete with units, labels, and a title. There should only be four sites on your figure. Here is an example of the plot for a different set of sites.
...
Remember to Render your document as an `HTML` and comment+push to GitHub your code and HTML document.  Use at least three commits that you push to GitHub.  

Potential issues with rendering:

  - You may have issues with your database connection when you render.  You can try running `dbDisconnect(con)` and/or restarting RStudio.

# Attribution

Include citations of any AI-generated assistance or discussion with classmates (per policy in the syllabus). Proper documentation of AI-generated assistance includes the prompt, the source (e.g., ChatGPT), and the significant parts of the response.  Proper documentation of a discussion with classmates includes listing their names and the components discussed.  
